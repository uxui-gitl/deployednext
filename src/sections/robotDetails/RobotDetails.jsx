    ...
    <div className="bg-[#fff] p-4">
      {sublinks.map((link, index) => (
        <span key={index} className="text-base leading-8  inline">
          <span className="text-[#0745d3] border-b-2 inline ">
            <Link
              href={link.url}
              target="_blank"
              className=" inline-flex align-middle hover:opacity-80 mb-4"
            >
              {link.title}
              <span>
                <Icon
      ...
      <div>
        ...
        <div className="mx-auto pt-[26px] pb-16">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
            {CardStack.Automation.map((cardCategory) =>
              cardCategory.sublinks.map((card) => (
                <Card
                  key={card._id}
